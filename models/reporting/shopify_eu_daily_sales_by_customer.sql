{{ config (
    alias = target.database + '_shopify_eu_daily_sales_by_customer'
)}}

{%- set schema_name,
        customer_tag_table_name
        = 'shopify_raw_eu', 'customer_tag' -%}
...
WITH orders AS 
    ...
    FROM {{ ref('shopify_eu_daily_sales_by_transaction') }}
